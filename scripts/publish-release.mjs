...
function api(endpoint, missingOK = false) {
  ...
  try {
    ...
    /* Report the actual CLI failure below. */
  }
...
function releaseFor(tag) {
  ...
  return (
    JSON.parse(gh(['api', `repos/${REPO}/releases?per_page=100`, '--paginate', '--slurp']))
      .flat()
      .find((release) => release.tag_name === tag) || null
  );
}
async function publish() {
  if (process.env.GITHUB_REPOSITORY !== REPO || !/^\d+$/.test(process.env.CI_RUN_ID || ''))
    throw Error('Expected the Starfall repository and a numeric CI run ID');
  const run = api(`actions/runs/${process.env.CI_RUN_ID}`),
    current = await metadata();
  if (!trustedRun(run, process.env.GITHUB_REPOSITORY, current.sha))
    throw Error(
      'Refusing to publish: CI must be successful, from a main push in this repository, and match the checked-out commit',
    );
  const isCurrent = () => api('git/ref/heads/main').object.sha === current.sha;
  if (!isCurrent()) {
    console.log('A newer main commit exists; this release is superseded.');
    return;
  }
  const message = execFileSync('git', ['log', '-1', '--format=%B'], { encoding: 'utf8' });
  if (skipRelease(message)) {
    console.log('Skip-Release trailer present; not publishing.');
    return;
  }
  const tag = `v${current.version}`,
    existing = releaseFor(tag),
    tagRef = api(`git/ref/tags/${tag}`, true);
  ...
  if (existing && !existing.draft) {
    console.log(`${tag} is already published; immutable assets were not changed.`);
    return;
  }
  ...
  try {
    ...
    const out = process.env.RELEASE_PUBLISH_DIR
      ? path.resolve(process.env.RELEASE_PUBLISH_DIR)
      : path.join(temp, 'out');
    ...
    if (tested.sha !== current.sha || tested.version !== current.version)
      throw Error('CI artifact provenance/version mismatch');
    for (const platform of ['mac', 'linux', 'win']) {
      const directory = path.join(incoming, `installers-${platform}`);
      await verifyInstallers(directory, tested.version, platform);
      for (const name of expectedAssets(tested.version, platform))
        await copyFile(path.join(directory, name), path.join(out, name));
    }
    ...
    await writeFile(
      path.join(out, 'release-info.json'),
      JSON.stringify({ ...tested, ciRun: run.html_url }, null, 2) + '\n',
    );
    const names = [...expectedAssets(tested.version), 'install.sh', 'icon.png', 'release-info.json'];
    await writeChecksums(out, names);
    names.push('checksums.txt');
    if (!isCurrent()) {
      console.log('New main commit arrived during download; not publishing a superseded build.');
      return;
    }
    if (!existing)
      gh([
        ...
        tested.sha,
        '--title',
        `Starfall Command ${tested.version}`,
        '--notes',
    ...
    for (const name of names) {
      ...
      if (!asset || asset.digest !== 'sha256:' + (await sha256(path.join(out, name))))
        throw Error('Remote asset digest mismatch: ' + name);
    }
    if (!isCurrent()) {
      console.log('New main commit arrived during upload; completed draft remains unpublished.');
      return;
    }
    ...
    if (published.draft || published.assets.length !== names.length)
      throw Error('Release did not become public with its full asset set');
