...
if (process.platform !== 'darwin')
  throw Error('This local .app packager requires macOS. npm start also works on other Electron-supported desktops.');
...
const dist = path.join(root, 'dist'),
  destination = path.join(dist, 'Starfall Command.app');
...
try {
  ...
  if (
    ...
    throw Error('Destination exists but is not a known Starfall build', { cause: error });
}
const staging = path.join(dist, '.starfall-build-' + randomUUID()),
  candidate = path.join(staging, 'Starfall Command.app'),
  backup = path.join(staging, 'previous.app');
...
try {
  ...
  const resources = path.join(candidate, 'Contents/Resources'),
    bundled = path.join(resources, 'app');
  await fs.mkdir(bundled);
  for (const item of ['index.html', 'src', 'electron', 'assets', 'README.md'])
    await fs.cp(path.join(root, item), path.join(bundled, item), { recursive: true });
  await fs.writeFile(
  ...
  set(plist, 'CFBundleDisplayName', 'Starfall Command');
  set(plist, 'CFBundleName', 'Starfall Command');
  set(plist, 'CFBundleIdentifier', 'com.starfall.command');
  set(plist, 'CFBundleShortVersionString', pkg.version);
  set(plist, 'CFBundleVersion', pkg.version);
  ...
  for (const entry of await fs.readdir(frameworks))
    if (entry.startsWith('Electron Helper') && entry.endsWith('.app')) {
      const helper = path.join(frameworks, entry, 'Contents/Info.plist'),
        suffix = entry.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
      set(helper, 'CFBundleDisplayName', entry.replace('Electron', 'Starfall'));
      set(helper, 'CFBundleName', entry.replace('Electron', 'Starfall').replace('.app', ''));
      set(helper, 'CFBundleIdentifier', 'com.starfall.command.' + suffix);
    }
  const icon = path.join(root, 'assets/generated/app-icon.png'),
    iconset = path.join(staging, 'Starfall.iconset');
  await fs.access(icon);
  await fs.mkdir(iconset);
  for (const size of [16, 32, 128, 256, 512])
    for (const scale of [1, 2]) {
      const n = String(size * scale),
        filename = `icon_${size}x${size}${scale === 2 ? '@2x' : ''}.png`;
      run('/usr/bin/sips', ['-z', n, n, icon, '--out', path.join(iconset, filename)]);
    }
  run('/usr/bin/iconutil', ['--convert', 'icns', '--output', path.join(resources, 'starfall.icns'), iconset]);
  set(plist, 'CFBundleIconFile', 'starfall.icns');
