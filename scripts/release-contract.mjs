...
export function expectedAssets(version, platform = null) {
  ...
  const assets = {
    ...
    linux: ['x86_64', 'arm64']
      .map((arch) => `${prefix}-linux-${arch}.AppImage`)
      .concat(['amd64', 'arm64'].map((arch) => `${prefix}-linux-${arch}.deb`)),
...
export function assertAssetSet(names, version, platform = null) {
  for (const name of expectedAssets(version, platform))
    if (!names.includes(name)) throw Error('Missing installer: ' + name);
  const extras = names.filter(
    (name) => /\.(dmg|zip|AppImage|deb|exe)$/.test(name) && !expectedAssets(version, platform).includes(name),
  );
...
export async function metadata(cwd = process.cwd()) {
  ...
  return {
    version: releaseVersion(pkg.version, Number(git(['rev-list', '--first-parent', '--count', 'HEAD']))),
