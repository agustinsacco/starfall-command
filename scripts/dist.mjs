...
if (!['mac', 'linux', 'win'].includes(platform))
  throw Error('Usage: node scripts/dist.mjs mac|linux|win [--arm64|--x64] [--dir]');
...
args.push(
  ...(options.some((arg) => ['--arm64', '--x64'].includes(arg))
    ? []
    : platform === 'win'
      ? ['--x64']
      : ['--arm64', '--x64']),
...
if (platform === 'mac' && env.CSC_LINK) {
  if (appleKeys.some((key) => !env[key]))
    throw Error('Signed distribution requires all five Apple signing/notarization variables');
