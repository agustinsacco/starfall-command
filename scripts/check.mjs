...
if (!html.includes("script-src 'self'") || /<script(?![^>]*src=)/.test(html))
  throw Error('Scripts must be local files with a strict CSP');
for (const [, file] of html.matchAll(/(?:src|href)="((?:src|assets)\/[^"]+)"/g)) await fs.access(file);
const pkg = JSON.parse(await fs.readFile('package.json', 'utf8')),
  lock = JSON.parse(await fs.readFile('package-lock.json', 'utf8'));
