...
protocol.registerSchemesAsPrivileged([
  { scheme: 'starfall', privileges: { standard: true, secure: true, supportFetchAPI: true, corsEnabled: true } },
]);
// Tests never use a player's actual profile or save files.
if (process.env.STARFALL_TEST_MODE === '1') {
  const dir = process.argv
    .find((a) => a.startsWith('--starfall-test-data='))
    ?.split('=')
    .slice(1)
    .join('=');
...
function setFullscreen(flag) {
  if (process.platform === 'darwin') {
    ...
    notify('desktop:fullscreen', win.isSimpleFullScreen());
...
async function closeFailed() {
  ...
  const { response } = await dialog.showMessageBox(win, {
    type: 'warning',
    title: 'Your operation was not saved',
    message: 'Starfall could not save the current operation.',
    detail: 'Keep playing to retry or export a save. Quitting now loses progress since the last successful save.',
    buttons: ['Keep playing', 'Quit without saving'],
    defaultId: 0,
    cancelId: 0,
    noLink: true,
  });
...
function requestClose() {
  ...
  const id = randomUUID(),
    timer = setTimeout(() => {
      if (closeRequest?.id === id) closeFailed();
    }, 12000);
  closeRequest = { id, timer };
  notify('desktop:before-close', id);
}
...
app
  ...
  .then(async () => {
    ...
    store = new SaveStore(path.join(app.getPath('userData'), 'operations'));
    await store.init();
    protocol.handle('starfall', (request) => {
      ...
      const allowed =
        relative === 'index.html' || /^(src|assets)\/[a-zA-Z0-9_./-]+\.(js|css|png|svg|webp)$/.test(relative);
    ...
    handle('saves:import', async () => {
      const chosen = await dialog.showOpenDialog(win, {
        title: 'Import Starfall operation',
        properties: ['openFile'],
    ...
    handle('saves:export', async (id) => {
      ...
      await store.atomicWrite(chosen.filePath, JSON.stringify(saved.record, null, 2));
      return true;
    });
    ...
    Menu.setApplicationMenu(
      Menu.buildFromTemplate([
          ...
          ? [
              {
                ...
                submenu: [
                  ...
                  { role: 'hideOthers' },
                  { type: 'separator' },
                  { label: 'Save and Quit', accelerator: 'Command+Q', click: () => app.quit() },
                ],
              },
            ]
          : []),
        ...
        { role: 'editMenu' },
        {
          label: 'Operation',
          submenu: [
            { label: 'Save', accelerator: 'F5', click: () => command('save') },
            { label: 'Game library', accelerator: 'CommandOrControl+L', click: () => command('library') },
            { type: 'separator' },
        ...
        },
        {
          label: 'View',
          submenu: [
            { label: 'Toggle Fullscreen', accelerator: 'F11', click: () => setFullscreen(!isFullscreen()) },
            { label: 'Field manual', click: () => command('help') },
          ],
        },
      ]),
    );
