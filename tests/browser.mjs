...
const server = createServer(async (req, res) => {
  ...
  if (!(file === 'index.html' || /^src\/[a-z-]+\.(js|css)$/.test(file))) {
    res.writeHead(404);
    res.end();
    return;
  }
...
try {
  const endpoint = await new Promise((resolve, reject) => {
    ...
    child.once('exit', (code) => {
      clearTimeout(timer);
      reject(Error('Chrome exited: ' + code));
    });
  });
  ...
  const wait = async (expression, message, ms = 5000) => {
  ...
  const mouse = async (x, y, button = 'left', modifiers = 0) => {
  ...
  const key = async (key, modifiers = 0, hold = 0) => {
  ...
  const worldClick = async (x, y, button = 'left') => {
    const p = await evaluate(
      `(()=>{const p=starfallApp.worldToScreen(${x},${y}),r=document.querySelector('#field').getBoundingClientRect();return{x:p.x+r.x,y:p.y+r.y};})()`,
    );
    await mouse(p.x, p.y, button);
  };
  const capture = async (name) => {
    const r = await call('Page.captureScreenshot', { format: 'png', captureBeyondViewport: false });
    await writeFile(path.join(shots, name + '.png'), Buffer.from(r.data, 'base64'));
  };
  ...
  await key('1', 2);
  await key('a');
  await worldClick(1040, 1110);
  assert.equal(
    await evaluate('starfallApp.selection.every(id => starfallApp.game.entity(id).order.type === "attackmove")'),
    true,
  );
  assert.equal(
    await evaluate('starfallApp.marks.at(-1)?.kind'),
    'attackmove',
    'attack-move click leaves an acknowledgement marker',
  );
  await key('s');
  await key('1');
  assert.equal(await evaluate('starfallApp.selection.length'), 2);
  ...
  assert.equal(
    await evaluate('starfallApp.selection.every(id => starfallApp.game.entity(id).order.type === "move")'),
    true,
  );
  assert.equal(await evaluate('starfallApp.marks.at(-1)?.kind'), 'move');
  const originalCamera = await evaluate('starfallApp.camera.x');
  await key('ArrowRight', 0, 220);
  assert.ok((await evaluate('starfallApp.camera.x')) > originalCamera);
  const wheel = (init) =>
    evaluate(
      `document.querySelector('#field').dispatchEvent(new WheelEvent('wheel',Object.assign({bubbles:true,cancelable:true,clientX:800,clientY:350},${JSON.stringify(init)})))`,
    );
  ...
  const picked = await evaluate(
    '(()=>{const e=starfallApp.game.entity(starfallApp.selection[0]);return e&&{id:e.id,type:e.type};})()',
  );
  assert.equal(picked?.type, 'drone', 'clicking the drone cluster selects a drone');
  const patch = await evaluate(
    `(()=>{const g=starfallApp.game,u=g.entity(${picked.id});return g.resources.filter(r=>r.type==='mineral'&&r.amount>0&&!g.entities.some(e=>e.hp>0&&Math.hypot(e.x-r.x,e.y-r.y)<40)).sort((a,b)=>Math.hypot(a.x-u.x,a.y-u.y)-Math.hypot(b.x-u.x,b.y-u.y)).map(r=>({id:r.id,x:r.x,y:r.y}))[0];})()`,
  );
  ...
  const site = await evaluate(
    '(()=>{const g=starfallApp.game;for(let y=1110;y<1490;y+=25)for(let x=300;x<800;x+=25)if(g.placement("relay",x,y,0).ok)return{x,y};})()',
  );
  assert.ok(site);
  ...
  assert.equal(
    await evaluate('starfallApp.marks.at(-1)?.kind'),
    'build',
    'placing a structure acknowledges the construction order',
  );
  ...
  const frozen = await evaluate('starfallApp.game.time');
  await sleep(250);
  ...
  const savedSimulation = await evaluate(
    `JSON.parse(localStorage.getItem('starfall-operation-v2/${operationId}')).snapshot.simulation`,
  );
  assert.ok(savedSimulation);
  await click('#resume-btn');
  await sleep(350);
  await key('p');
  await click('#load-btn');
  ...
  await sleep(250);
  assert.ok(
    (await evaluate('getComputedStyle(document.querySelector(".cmd")).backgroundImage')).includes('rgb(232, 238, 233)'),
  );
  ...
  assert.equal(
    await evaluate(
      '(async()=>{const old=Storage.prototype.setItem;Storage.prototype.setItem=()=>{throw Error("denied")};try{return await starfallApp.saveGame();}finally{Storage.prototype.setItem=old;}})()',
    ),
    false,
  );
  assert.equal(
    await evaluate(
      `(async()=>{const id=starfallApp.operation.id;localStorage.setItem('starfall-operation-v2/'+id,'broken');localStorage.removeItem('starfall-operation-v2/'+id+'/backup');return await starfallApp.loadGame(id);})()`,
    ),
    false,
  );
