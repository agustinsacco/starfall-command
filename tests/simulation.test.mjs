...
const advance = (g, seconds) => {
  for (let i = 0; i < Math.ceil(seconds / 0.1) && !g.result; i++) g.update(0.1);
};
const quiet = () => {
  const g = new Game();
  g.ai = () => {};
...
const rich = (g) => {
  g.teams[0].minerals = 10000;
  g.teams[0].gas = 10000;
};
const findSite = (g, type, worker = g.own(0, 'drone')[0]) => {
  for (let y = 1030; y < 1530; y += 28)
    for (let x = 300; x < 850; x += 28) {
...
test('abandoned construction can be resumed by another drone; cancellation refunds 75%', () => {
  ...
  advance(g, 45);
  assert.equal(b.complete, true);
  const p2 = findSite(g, 'relay'),
    money = g.teams[0].minerals;
  const relay = g.build(u.id, 'relay', p2.x, p2.y);
  assert.ok(relay);
  assert.equal(g.cancelBuild(relay.id), true);
  assert.equal(g.teams[0].minerals, money - 90 + Math.floor(90 * 0.75));
...
test('gas needs an extractor; right-clicking the completed extractor assigns harvesting', () => {
  ...
  assert.ok(b);
  assert.equal(b.x, vent.x);
  assert.equal(b.vent, vent.id);
  advance(g, 42);
  assert.equal(b.complete, true);
...
test('changing resource assignments deposits existing cargo without converting its type', () => {
  const g = quiet(),
    u = g.own(0, 'drone')[0];
  const vent = g.resources.find((r) => r.type === 'gas');
  const extractor = g.add('refinery', 0, vent.x, vent.y);
  extractor.vent = vent.id;
  ...
  const minerals = g.teams[0].minerals,
    gas = g.teams[0].gas;
  g.worker(u, 0.1);
...
test('research has prerequisites, costs, exclusivity, and a three-level cap', () => {
  const g = quiet();
  rich(g);
  const lab = g.add('lab', 0, 870, 1340),
    lab2 = g.add('lab', 0, 970, 1340);
...
test('combat, armor, healing, hold orders, siege splash, and aircraft targeting rules work', () => {
  const g = quiet();
  const a = g.add('ranger', 0, 1200, 920),
    b = g.add('ranger', 1, 1300, 920);
  g.updateVision();
  const hp = b.hp;
  advance(g, 1);
  assert.ok(b.hp < hp);
  const medic = g.add('mender', 0, 1190, 950);
  a.hp = 20;
  g.order([a.id], 'hold', 0, 0);
  g.order([b.id], 'move', 1600, 800);
  advance(g, 3);
  assert.ok(a.hp > 20);
  const tank = g.add('tank', 0, 1000, 1400),
    e1 = g.add('ranger', 1, 1250, 1400),
    e2 = g.add('ranger', 1, 1250, 1425);
  g.order([tank.id], 'siege', 0, 0);
  g.updateVision();
  ...
  const air = g.add('wraith', 1, 1020, 1400),
    airHp = air.hp;
  e1.hp = 0;
  e2.hp = 0;
  tank.cool = 0;
  ...
  g.damage(medic, 10, b);
  assert.equal(medic.hp, before - 8);
});
...
test('move-only does not auto-attack, and unseen enemies cannot be targeted', () => {
  ...
  const target = g.add('ranger', 1, u.x + 30, u.y);
...
test('saving restores the actual simulation: resources, orders, queues, upgrades, exploration, and RNG', () => {
  ...
  const bad = JSON.parse(save);
  bad.entities[0].type = '<invalid>';
...
test('victory/defeat is based on all structures, and a finished game is frozen', () => {
  for (const [loser, result] of [
    [1, 'victory'],
    [0, 'defeat'],
  ]) {
    const g = quiet();
    g.own(loser, 'hq')[0].hp = 0;
    advance(g, 0.1);
    assert.equal(g.result, null, 'HQ loss alone is not defeat');
    g.own(loser)
      .filter((e) => D[e.type].building)
      .forEach((e) => {
        e.hp = 0;
      });
    advance(g, 0.1);
...
test('difficulties grant a real grace period and stay ordered from cadet to veteran', { timeout: 180000 }, () => {
  ...
  for (const d of ['easy', 'normal', 'hard']) {
    ...
    for (let i = 0; i < 9000 && !g.result; i++) {
      ...
      if (i % 25 === 0)
        maxArmy = Math.max(maxArmy, g.own(1).filter((e) => !D[e.type].building && e.type !== 'drone').length);
    }
  ...
  assert.ok(
    outcome.hard.defeat < outcome.normal.defeat && outcome.normal.defeat < outcome.easy.defeat,
    JSON.stringify(outcome),
  );
  assert.ok(
    outcome.easy.defeat > 320,
    'a fully passive player gets over five minutes on cadet: ' + outcome.easy.defeat,
  );
...
test('issued orders return an acknowledgement kind for interface feedback', () => {
  ...
  const drone = g.own(0, 'drone')[0],
    mineral = g.resource(drone.order.target);
