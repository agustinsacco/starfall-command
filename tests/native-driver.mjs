...
export async function launchNative(profile, executable = null) {
  const env = { ...process.env, STARFALL_TEST_MODE: '1' }; delete env.ELECTRON_RUN_AS_NODE;
  const args = executable ? [] : ['.'];
  const binary = executable || require('electron'), launchArgs = [...args, '--remote-debugging-port=0', `--starfall-test-data=${profile}`];
  const child = spawn(binary, launchArgs, { env, stdio: ['ignore', 'pipe', 'pipe'] });
  const exited = new Promise(resolve => child.once('exit', (code, signal) => resolve({ code, signal })));
  let logs = '', ws, nextId = 0; const pending = new Map(), errors = [];
  child.stdout.on('data', b => { logs += b.toString(); }); child.stderr.on('data', b => { logs += b.toString(); });
  const until = Date.now() + 20000; let endpoint;
  while (Date.now() < until && child.exitCode === null) { endpoint = logs.match(/DevTools listening on (ws:\/\/[^\s]+)/)?.[1]; if (endpoint) break; await sleep(80); }
  if (!endpoint) { child.kill(); throw Error('Electron failed to start: ' + logs); }
  const port = new URL(endpoint).port; let target;
  for (let i = 0; i < 100; i++) {
    const targets = await (await fetch(`http://127.0.0.1:${port}/json/list`)).json(); target = targets.find(t => t.type === 'page');
    if (target) break; await sleep(80);
  }
  if (!target) { child.kill(); throw Error('No Electron page: ' + logs); }
  ...
  const driver = { child, exited, call, evaluate, wait, click, key, stop, duplicateInstance, errors, get logs() { return logs; } };
  try { await wait('typeof starfallApp !== "undefined" && typeof starfallDesktop !== "undefined"', 'renderer and isolated preload'); }
