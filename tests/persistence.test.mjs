...
function snapshot(g = new Game()) {
  return {
    ...
    view: {
      ...
      selection: g.own(0, 'ranger').map((e) => e.id),
      groups: { 1: g.own(0, 'drone').map((e) => e.id) },
      speed: 1.5,
      sound: true,
...
async function setup(t) {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'starfall-save-test-'));
...
test('v2 restores every simulation field byte-for-byte, including paths, visibility timing, effects and AI', () => {
  ...
  g.teams[0].minerals += 500;
  g.build(drone.id, 'relay', 630, 1260);
  tick(g, 13);
  const save = g.save(),
    restored = Game.load(save);
  ...
  for (let i = 0; i < 1400; i++) {
    ...
    if (i % 200 === 0) assert.equal(restored.save(), g.save());
  }
...
test('portable envelope preserves complete view, commands, selection and checks its checksum', async () => {
  const data = snapshot(),
    id = randomUUID(),
    made = await F.makeRecord({ id, name: 'North Ridge', snapshot: data });
  ...
  const changed = structuredClone(made.record);
  changed.snapshot.view.speed = 2;
  await assert.rejects(F.parseRecord(JSON.stringify(changed)), /integrity/);
  const badView = structuredClone(data);
  badView.view.camera.x = -10;
...
test('malformed simulation fields cannot crash the game after being accepted', () => {
  ...
  const mutations = [
    (p) => {
      p.entities[0].order = null;
    },
    (p) => {
      p.entities[0].path = [{ x: -5, y: 10 }];
    },
    (p) => {
      p.entities[1].id = p.entities[0].id;
    },
    (p) => {
      p.nextId = 1;
    },
    (p) => {
      p.teams[0].stats.kills = '<script>';
    },
    (p) => {
    ...
    (p) => {
      p.entities[0].queue = [{ kind: 'unit', type: 'tank', progress: 0.2, time: 5, m: 1, g: 0 }];
    },
  ];
  for (const mutate of mutations) {
    const p = structuredClone(base);
    mutate(p);
    assert.throws(() => Game.load(JSON.stringify(p)), /Invalid/);
  }
...
test('legacy browser saves migrate without overwriting or losing game resources', async () => {
  const g = new Game();
  tick(g, 127);
  const legacy = JSON.parse(g.save());
  legacy.version = 1;
  ...
  const raw = JSON.stringify({
    ...
    groups: { 1: [g.entities[0].id] },
    speed: 2,
  });
  const migrated = F.migrateLegacy(raw),
    loaded = F.validateSnapshot(migrated);
...
test('separate named operations persist through a fresh store instance', async (t) => {
  ...
  const g = new Game('hard', 77);
  tick(g, 40);
  await store.write({ id: b, name: 'Second Expedition', snapshot: snapshot(g) });
  const reopened = new SaveStore(dir),
    rows = await reopened.list();
  assert.equal(rows.length, 2);
  assert.equal((await reopened.read(a)).summary.time, 0);
...
test('concurrent saves are serialized, with monotonic revisions and a last-known-good backup', async (t) => {
  const { store, dir } = await setup(t),
    id = randomUUID();
  await Promise.all(
    Array.from({ length: 8 }, (_, i) => store.write({ id, name: 'Revision ' + i, snapshot: snapshot() })),
  );
  const latest = await store.read(id);
  assert.equal(latest.record.revision, 8);
  assert.equal(latest.record.name, 'Revision 7');
...
test('a truncated primary recovers from backup; malformed files never erase another game', async (t) => {
  ...
  assert.equal(restored.record.name, 'Before');
  ...
  await fs.writeFile(path.join(dir, id + '.json.bak'), '{bad');
...
test('failed final write retains the previous save and can be retried', async (t) => {
  ...
  store.atomicWrite = async (file, data) => {
    if (file.endsWith('.json')) throw Error('Disk full');
...
test('rename, portable export/import, legacy import and deletion are isolated', async (t) => {
  ...
  assert.equal((await store.read(id)).record.name, 'Renamed');
  const exported = await store.export(id),
    imported = await store.import(exported);
  assert.notEqual(imported.id, id, 'import must create a new game, not overwrite its source');
  assert.equal(
    (await store.read(imported.id)).record.snapshot.simulation,
    (await store.read(id)).record.snapshot.simulation,
  );
  const legacy = await store.import(JSON.stringify({ game: new Game().save() }));
  assert.equal((await store.list()).length, 3);
  await store.delete(id);
  assert.equal((await store.list()).length, 2);
  assert.ok(await store.read(legacy.id));
...
test('path traversal, unsupported formats and invalid names are rejected', async (t) => {
  ...
  for (const id of ['../escape', '/tmp/x', '__proto__', randomUUID() + '/../../x']) {
    ...
    await assert.rejects(store.delete(id), /ID/);
  }
