...
import { REPO, releaseVersion, assertVersion, expectedAssets, assertAssetSet, trustedRun, skipRelease, sha256, writeChecksums } from '../scripts/release-contract.mjs';
...
test('release versions are deterministic and stable semver', () => {
  ...
  for (const bad of ['2.0.0-beta', '2.0', '../../escape', '02.0.0', '2.0.0;exit']) assert.throws(() => assertVersion(bad));
...
test('only green main pushes from this repository and exact SHA are releasable', () => {
  const sha = 'a'.repeat(40), run = { name: 'CI', event: 'push', conclusion: 'success', head_branch: 'main', head_repository: { full_name: REPO }, head_sha: sha };
  assert.equal(trustedRun(run, REPO, sha), true);
  for (const patch of [{ name: 'Other workflow' }, { event: 'pull_request' }, { event: 'workflow_dispatch' }, { conclusion: 'failure' }, { conclusion: 'cancelled' }, { head_branch: 'feature' }, { head_repository: { full_name: 'attacker/starfall-command' } }, { head_sha: 'b'.repeat(40) }]) assert.equal(trustedRun({ ...run, ...patch }, REPO, sha), false);
...
test('skip-release is a git trailer, not a phrase that can suppress its own documentation', () => {
  ...
  assert.equal(skipRelease('Document trailers\n\nExample:\n    Skip-Release: true\n\nUse only for deliberate skips.\n'), false);
...
test('release requires all nine installers, and catches wrong architecture names and stale versions', () => {
  const names = expectedAssets('2.0.2'); assert.equal(names.length, 9);
  assertAssetSet(names, '2.0.2');
  for (const missing of names) assert.throws(() => assertAssetSet(names.filter(n => n !== missing), '2.0.2'), /Missing installer/);
...
test('checksums cover exact bytes and reject path escapes', async () => {
  ...
  try {
    ...
    assert.equal(await fs.readFile(path.join(dir, 'checksums.txt'), 'utf8'), (await sha256(path.join(dir, 'one.zip'))) + '  one.zip\n');
    await assert.rejects(writeChecksums(dir, ['../escape']), /Unsafe/);
  } finally { await fs.rm(dir, { recursive: true, force: true }); }
});

test('installer selects real electron-builder asset names without network access', { skip: process.platform === 'win32' }, () => {
  for (const [platform, arch, expected] of [
    ['Darwin', 'arm64', 'mac-arm64.dmg'], ['Darwin', 'x86_64', 'mac-x64.dmg'],
    ['Linux', 'amd64', 'linux-x86_64.AppImage'], ['Linux', 'aarch64', 'linux-arm64.AppImage']
  ]) {
    const selected = execFileSync('sh', ['scripts/install.sh', '--print-asset', platform, arch, '2.0.2'], { encoding: 'utf8' }).trim();
...
test('installer fresh install/update preserves saves; corrupt or missing checksums leave the old app intact', { skip: process.platform === 'win32' }, async () => {
  const temp = await fs.mkdtemp(path.join(os.tmpdir(), 'starfall-install-test-'));
  try {
    const bin = path.join(temp, 'tools'), source = path.join(temp, 'source'), prefix = path.join(temp, 'home', 'local games');
    await fs.mkdir(bin); await fs.mkdir(source);
    ...
    const env = { ...process.env, HOME: path.join(temp, 'home'), XDG_CONFIG_HOME: path.join(temp, 'home/.config'), PATH: bin + path.delimiter + process.env.PATH, STARFALL_PREFIX: prefix, STARFALL_VERSION: 'v2.0.2', FIXTURE_SOURCE: source };
    ...
    let result = install(); assert.equal(result.status, 0, result.stderr);
    const target = path.join(prefix, 'bin/starfall-command');
    assert.equal(await fs.readFile(target, 'utf8'), 'first fixture game');
    assert.ok((await fs.readFile(path.join(prefix, 'share/applications/starfall-command.desktop'), 'utf8')).includes(`Exec="${target}"`));
    const saves = path.join(temp, 'home', '.config', 'Starfall Command', 'operations'); await fs.mkdir(saves, { recursive: true });
    await fs.writeFile(path.join(saves, 'sentinel.json'), 'player progress');
    await fs.writeFile(path.join(source, asset), 'second fixture game'); await writeChecksums(source, [asset, 'icon.png']);
    ...
    result = install(); assert.notEqual(result.status, 0); assert.match(result.stderr, /Checksum mismatch/);
    assert.equal(await fs.readFile(target, 'utf8'), 'second fixture game');
    await fs.rm(path.join(source, 'checksums.txt'));
    result = install(); assert.notEqual(result.status, 0); assert.match(result.stderr, /checksums are unavailable/);
    assert.equal(await fs.readFile(target, 'utf8'), 'second fixture game');
    const unrelated = path.join(temp, 'unrelated'); await fs.mkdir(path.join(unrelated, 'bin'), { recursive: true });
    await fs.writeFile(path.join(unrelated, 'bin/starfall-command'), 'not our application');
    await writeChecksums(source, [asset, 'icon.png']);
    const refused = spawnSync('sh', ['scripts/install.sh'], { env: { ...env, STARFALL_PREFIX: unrelated }, encoding: 'utf8' });
    assert.notEqual(refused.status, 0); assert.match(refused.stderr, /not a known Starfall/);
