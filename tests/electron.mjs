...
async function capture(name) {
  const shot = await driver.call('Page.captureScreenshot', { format: 'png' });
...
async function quitThroughUI() {
  ...
  const result = await Promise.race([
    driver.exited,
    sleep(18000).then(() => {
      throw Error('App did not finish save-on-quit: ' + driver.logs);
    }),
  ]);
...
try {
  ...
  assert.ok(
    Math.abs(viewport.w - viewport.sw) <= 2 && Math.abs(viewport.h - viewport.sh) <= 2,
    'Fullscreen content must fill the physical display in CSS pixels',
  );
  ...
  assert.equal(
    await driver.evaluate('fetch("starfall://app/src/..%2fnode_modules%2felectron%2findex.js").then(r=>r.status)'),
    403,
    'Encoded traversal must not escape the public asset directories',
  );
  ...
  await driver.click('#fullscreen-btn');
  await driver.wait(
    '(async()=> (await starfallDesktop.info()).fullscreen && Math.abs(innerWidth-screen.width)<=2 && Math.abs(innerHeight-screen.height)<=2)()',
    'returning to display-filling fullscreen',
  );
  ...
  console.log(
    'PASS actual fullscreen Electron window, single-instance ownership, isolated preload, restricted protocol and IPC',
  );

  await driver.evaluate('document.querySelector("#operation-name").value="Native Expedition Alpha"');
  await driver.click('#deploy-btn');
  await driver.wait(
    'starfallApp.operation?.name === "Native Expedition Alpha" && !starfallApp.paused',
  ...
  await driver.evaluate(
    `(()=>{const g=starfallApp.game;g.teams[0].minerals+=500;g.train(g.own(0,'barracks')[0].id,'ranger');g.order(g.own(0,'ranger').map(e=>e.id),'attackmove',1120,1120);g.build(g.own(0,'drone')[0].id,'relay',420,1090);})()`,
  );
  await driver.wait('starfallApp.game.time >= 6', 'real-time simulation with in-flight orders');
  await driver.key('F2');
  await driver.key('1', 2);
  await driver.key('ArrowRight', 0, 150);
  await driver.click('#speed-btn');
  await driver.key('a');
  ...
  driver = await launchNative(profile, process.env.STARFALL_EXECUTABLE);
  await driver.wait(
    '!!document.querySelector("#continue-btn") && !document.querySelector("#continue-btn").hidden',
    'continue after a new process launch',
  );
  ...
  await driver.evaluate(
    'document.querySelector("#operation-name").value="Native Expedition Beta";document.querySelector("#operation-seed").value="104729"',
  );
  await driver.click('[data-diff="hard"]');
  await driver.click('#deploy-btn');
  await driver.wait(
    'starfallApp.operation?.name === "Native Expedition Beta" && !starfallApp.paused',
    'independent second game',
  );
  const beta = await driver.evaluate('starfallApp.operation.id');
  assert.notEqual(beta, alpha);
  const beforeAuto = await driver.evaluate('starfallApp.operation.revision');
  await driver.wait(
    `(()=>{if(starfallApp.paused)throw Error('Running autosave fixture was paused; focused='+document.hasFocus());return starfallApp.operation.revision > ${beforeAuto} && starfallApp.operation.time > 10;})()`,
    'automatic on-disk save without clicking Save',
    40000,
  );
  const rows = await driver.evaluate('starfallDesktop.saves.list()');
  ...
  await driver.wait(
    `document.querySelector('[data-save-id="${alpha}"] h3')?.textContent === 'Alpha <Vanguard>'`,
    'safe operation rename',
  );
  ...
  await driver.click('#load-btn');
  await driver.wait(
    'document.querySelectorAll(".save-card").length === 2',
    'library fully loaded before checkpoint action',
  );
  ...
  const checkpoint = (await driver.evaluate('starfallDesktop.saves.list()')).find(
    (r) => r.name === 'Independent checkpoint',
  );
  ...
  await driver.click(`[data-save-id="${checkpoint.id}"] [data-action="delete"]`);
  await driver.click('#confirm-yes');
