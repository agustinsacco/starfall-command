...
(() => {
  ...
  class BrowserSaves {
    ...
    async readInternal(id) {
      ...
      let error;
      try {
        if (!raw) throw Error('Operation not found');
        const parsed = await F.parseRecord(raw);
        if (parsed.record.id !== id) throw Error('Save identity mismatch');
        return { ...parsed, recovered: false };
      } catch (e) {
        error = e;
      }
      if (backup) {
        const parsed = await F.parseRecord(backup);
        if (parsed.record.id !== id) throw Error('Backup identity mismatch');
      ...
      return this.serial(async () => {
        ...
        for (const id of ids) {
          try {
            ...
            list.push({ ...r.summary, recovered: r.recovered });
          } catch (error) {
            list.push({ id, name: 'Unreadable operation', corrupt: true, error: error.message, updatedAt: '' });
          }
        }
    ...
    read(id) {
      return this.serial(() => this.readInternal(id));
    }
    write(input) {
      return this.serial(async () => {
        const key = this.key(input.id);
        let previous = null;
        ...
        localStorage.setItem(key, JSON.stringify(made.record));
        return made.summary;
      });
    }
    async rename(id, name) {
      const r = await this.read(id);
      return this.write({ id, name, snapshot: r.record.snapshot });
    }
    delete(id) {
      return this.serial(async () => {
        const key = this.key(id);
    ...
    async importLegacy(raw) {
      ...
      if (parsed?.format === F.FORMAT) {
        const r = await F.parseRecord(raw);
        name = r.record.name;
        snapshot = r.record.snapshot;
      } else {
    ...
    async exportFile(id) {
      const r = await this.read(id),
        blob = new Blob([JSON.stringify(r.record, null, 2)], { type: 'application/json' });
      const url = URL.createObjectURL(blob),
        a = document.createElement('a');
      a.href = url;
      a.download = r.record.name.replace(/[^a-zA-Z0-9 _-]/g, '_') + '.starfall.json';
