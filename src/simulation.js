...
class Game{
 ...
 order(ids,type,x,y,target=null){let units=ids.map(id=>this.entity(id)).filter(Boolean);const cols=Math.ceil(Math.sqrt(units.length));let i=0;for(const u of units){const d=D[u.type];if(d.building){if(['move','attackmove','context'].includes(type))u.rally={x:clamp(x,20,W-20),y:clamp(y,20,H-20)};continue;}if(type==='siege'){if(u.type==='tank'){u.siege=!u.siege;u.path=[];u.order={type:'hold'};}continue;}let next=type,tx=x,ty=y,t=target?this.entity(target)||this.resource(target):null;if(type==='context'){if(t?.amount!==undefined&&u.type==='drone'){if(t.type==='gas'&&!this.own(u.team,'refinery',true).some(b=>b.vent===t.id)){this.emit('Build an Extractor on this vent first',u.team,true);continue;}next='gather';}else if(t?.team!==undefined&&t.team!==u.team)next='attack';else if(t?.team===u.team&&D[t.type].building&&u.type==='drone'){if(t.type==='refinery'&&t.complete){next='gather';target=t.vent;}else next=t.complete?'repair':'build';}else next='move';}
   ...
   u.order={type:next,x:tx,y:ty,target:target||undefined};u.path=[];u.pathGoal=null;
  }
 }
 ...
 update(dt){if(this.result)return;dt=clamp(dt,0,.2);this.time+=dt;this.visionClock-=dt;if(this.visionClock<=0){this.updateVision();this.visionClock=.25;}
  ...
  const destroyed=this.entities.some(e=>e.hp<=0&&D[e.type].building);this.entities=this.entities.filter(e=>e.hp>0);if(destroyed)this.rebuildGrid();this.effects=this.effects.filter(f=>(f.life-=dt)>0);this.aiClock-=dt;if(this.aiClock<=0){this.aiClock=this.difficulty==='hard'?1.3:this.difficulty==='easy'?3:2;this.ai();}
 ...
 ai(){const team=1,t=this.teams[team],hq=this.own(team,'hq',true)[0];if(!hq)return;const workers=this.own(team,'drone'),army=this.own(team).filter(e=>!D[e.type].building&&e.type!=='drone');const gas=this.own(team,'refinery',true)[0];
  ...
  const wantWorkers=this.difficulty==='easy'?10:this.difficulty==='hard'?17:14;if(workers.length+hq.queue.length<wantWorkers&&hq.queue.length<2)this.train(hq.id,'drone');
  const supply=this.supply(team);let type=null;const has=k=>this.own(team,k).length;const plans=['refinery','lab','factory','barracks','airfield','turret'];if(supply.cap-supply.used<5&&supply.cap<200&&!this.own(team).some(e=>e.type==='relay'&&!e.complete))type='relay';else if(!has('barracks'))type='barracks';else if(this.time>20+this.aiStage*18&&this.aiStage<plans.length){const p=plans[this.aiStage];if((p==='barracks'&&has(p)<2)||!has(p))type=p;else this.aiStage++;}
  ...
  if(this.time>=this.nextAttack&&army.length>=(this.difficulty==='easy'?10:7)){this.aiWave++;const known=this.own(0).filter(e=>D[e.type].building&&this.isVisible(e,1));const target=known[0]||{x:440,y:1280};this.order(army.map(e=>e.id),'attackmove',target.x,target.y);this.nextAttack=this.time+({easy:130,normal:95,hard:65}[this.difficulty]||95);this.emit('Long-range sensors: hostile strike force inbound.',0,true);}
 }
 save(){
  const copy={};
